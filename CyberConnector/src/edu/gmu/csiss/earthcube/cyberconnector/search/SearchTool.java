...
public class SearchTool {
	...
	public static SearchResponse searchVDP(SearchRequest req){
		...
		StringBuffer sql = new StringBuffer("select * from products where name like '%").append(req.searchtext).append("%'  ");
...
//		sql.append(" LIMIT ").append(req.getRecordsperpage()).append(";");
	...
	public static String constructCSWRequest(SearchRequest req){
		...
		int startpos = (req.pageno-1)*req.recordsperpage + 1;
	 ...
	 * @return
	 */
	public static SearchResponse searchPyCSW(SearchRequest req){
...
//				}
	...
	public static SearchResponse searchCSW(SearchRequest req){
		
		SearchResponse resp = null;
		
		if("1".equals(req.csw)){
			
			resp = SearchTool.searchPyCSW(req);
			
		}else if("0".equals(req.csw)){
			
			resp = SearchTool.searchCWIC(req);
	...
	public static SearchResponse search(SearchRequest req){
		...
		if(resp!=null){
			...
			for(int i=0, len=resp.getProducts().size(); i<len; i++){
				...
				if(p.getAccessurl().startsWith(SysDir.CACHE_DATA_URLPREFIX)){
